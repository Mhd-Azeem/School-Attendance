...
export default function App(){const {profile}=useAuth(),admin=profile?.role==='SECTION_HEAD';return <Routes><Route path="/login" element={<Login/>}/><Route element={<Protected/>}><Route index element={<Dashboard/>}/><Route path="attendance" element={<Attendance/>}/><Route path="period-attendance" element={<PeriodAttendance/>}/><Route path="history" element={<History/>}/><Route path="students" element={admin?<Students/>:<Navigate to="/"/>}/><Route path="teachers" element={admin?<Teachers/>:<Navigate to="/"/>}/><Route path="reports" element={admin?<Reports/>:<Navigate to="/"/>}/><Route path="calendar" element={admin?<Calendar/>:<Navigate to="/"/>}/><Route path="audit" element={admin?<Audit/>:<Navigate to="/"/>}/><Route path="settings" element={admin?<Settings/>:<Navigate to="/"/>}/><Route path="profile" element={<Profile/>}/></Route><Route path="*" element={<Navigate to="/" replace/>}/></Routes>}
