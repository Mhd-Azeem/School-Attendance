...
export function Attendance(){
 ...
 useEffect(()=>{if(!classId)return;(async()=>{
  ...
  try{
   ...
   setStudents(s)
   const draft=loadDraft(classId,date)
   ...
   try{
    const h=await api<{sessions:any[]}>(`/api/history?class_id=${encodeURIComponent(classId)}&from=${date}&to=${date}`)
 ...
 useEffect(()=>{if(classId&&Object.keys(statuses).length)saveDraft({classId,date,statuses,updatedAt:new Date().toISOString()})},[classId,statuses,date])
 ...
 async function submit(){
  ...
  if(!confirm(`${already?'Update and resubmit':'Submit'} attendance for Class ${selected?.display_name} for ${prettyDate(date)}?\n\nPresent: ${count.present}  Absent: ${count.absent}`))return
  setBusy(true);setMessage('')
  try{
   await api('/api/attendance',{method:'POST',body:JSON.stringify({class_id:classId,date,records:students.map(s=>({student_id:s.id,status:statuses[s.id] as AttendanceStatus}))})})
   clearDraft(classId,date);setAlready(true);setMissingIds(new Set())
   setMessage(savedIds.size?'Student attendance updated and resubmitted ✓':'Attendance submitted ✓')
  }catch(e){
 ...
 return <>
  <div className="screen-title-row student-attendance-title"><div><h1>Student Attendance</h1><p>{prettyDate(date)}</p></div><div className="attendance-top-controls"><label>Date<input type="date" value={date} onChange={e=>setDate(e.target.value||today)}/></label><select value={classId} onChange={e=>setClassId(e.target.value)} aria-label="Class">{classes.map(c=><option value={c.id} key={c.id}>{c.display_name}</option>)}</select></div></div>
  ...
  {already&&<section className="success-panel editable-register"><CheckCircle2/><h2>Attendance Submitted</h2><p>This register can be edited and resubmitted. Change any Present/Absent status below, then tap Update & Resubmit.</p></section>}
  <>
