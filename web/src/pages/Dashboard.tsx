...
export function StudentSummary(){
 ...
 const [students,setStudents]=useState<any[]>([]),[dashboard,setDashboard]=useState<Summary[]>([]),[details,setDetails]=useState<any[]>([]),[error,setError]=useState(''),[expanded,setExpanded]=useState<string|null>(null)
 useEffect(()=>{Promise.all([api<{students:any[]}>('/api/students'),api<{classes:Summary[]}>(`/api/dashboard/today?date=${today}&_=${Date.now()}`)]).then(async([s,d])=>{const ds=d.classes.map((c:any)=>({...c,id:String(c.class_id??c.id)}));setStudents(s.students);setDashboard(ds);if(type==='present'||type==='absent'){const out:any[]=[];for(const c of ds.filter(x=>x.session_id)){try{const x:any=await api(`/api/attendance/${c.session_id}`);for(const r of x.records||[])if(String(r.status).toLowerCase()===type)out.push({...r,class_id:c.id,class_name:c.display_name})}catch{}}setDetails(out)}}).catch(()=>setError('Could not load student list.'))},[type,today])
 ...
 const list=type==='total'?totalList:details
 const groups=dashboard.map(c=>({cls:c,items:list.filter((s:any)=>String(s.class_id)===String(c.id))}))
 function accordion(){return <section className="class-student-groups">{groups.map(g=><section className="class-student-card" key={g.cls.id}><header className="expandable-class-header" role="button" tabIndex={0} onClick={()=>setExpanded(x=>x===g.cls.id?null:g.cls.id)} onKeyDown={e=>{if(e.key==='Enter'||e.key===' '){e.preventDefault();setExpanded(x=>x===g.cls.id?null:g.cls.id)}}}><div><strong>{g.cls.display_name}</strong><small>{g.items.length} student{g.items.length===1?'':'s'}</small></div><ChevronDown className={expanded===g.cls.id?'rotated':''}/></header>{expanded===g.cls.id&&<div className="class-student-body">{g.items.map((s:any,i:number)=><article key={(s.student_id||s.id||i)+'-'+i}><span className="student-index">{i+1}</span><strong className="student-admission">{s.admission_number}</strong><div className="student-main-name"><strong>{s.full_name||s.student_name}</strong></div></article>)}{!g.items.length&&<div className="class-empty-students">No students in this category.</div>}</div>}</section>)}</section>}
 return <><div className="screen-title-row"><div><h1>{title}</h1><p>{prettyDate(today)}</p></div></div>{error&&<div className="error">{error}</div>}
 {(type==='total'||type==='present'||type==='absent')&&accordion()}
 {type==='submitted'&&<section className="class-student-groups">{dashboard.map(c=>{const done=submittedRows.some(x=>x.id===c.id);return <section className="class-student-card" key={c.id}><header className="expandable-class-header" role="button" tabIndex={0} onClick={()=>setExpanded(x=>x===c.id?null:c.id)}><div><strong>{c.display_name}</strong><small>{done?'Submitted':'Not submitted'}</small></div><ChevronDown className={expanded===c.id?'rotated':''}/></header>{expanded===c.id&&<div className="class-empty-students">{done?`${Number(c.marked??0)}/${c.total} students marked · ${c.present} present · ${c.absent} absent`:'Attendance has not been submitted today.'}</div>}</section>})}</section>}
 </>}
